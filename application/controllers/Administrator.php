...
class Administrator extends CI_Controller
{
	...
	public function seller()
	{
		...
		$totalRec = $this->Administrator_Model->get_seller(false, $conditions);

		$config['target']      = '#dataList';
		$config['base_url']    = base_url('administrator/ajaxPaginationseller/');
		...
		$data['seller'] = $this->Administrator_Model->get_seller(false, $conditions);


		$this->load->view('administrator/header-script');
		$this->load->view('administrator/header');
		$this->load->view('administrator/header-bottom');
		$this->load->view('administrator/sellerlist', $data);
		$this->load->view('administrator/footer');
	}

	function ajaxPaginationseller()
	{
		...
		$this->load->view('administrator/ajaxdataseller', $data, false);
	}

	public function add_seller($page = 'addseller')
	{
		if (!file_exists(APPPATH . 'views/administrator/' . $page . '.php')) {
			show_404();
		}
		...
		$this->form_validation->set_rules('email', 'Email', 'required|is_unique[seller.email]');

		$this->form_validation->set_rules('number', 'Number', 'required|is_unique[seller.mobile]');


		$this->form_validation->set_rules('gst', 'Gst', 'required|is_unique[seller.gst_number]');

		$this->form_validation->set_rules('gst', 'Gst', 'required|is_unique[seller.gst_number]');

		$this->form_validation->set_rules('password', 'Password', 'required');

		$this->form_validation->set_rules('confirmpasswords', 'Confirmpasswords', 'required');
		...
		if ($this->form_validation->run() === FALSE) {
			...
			$this->Administrator_Model->add_seller();

			//Set Message
			$this->session->set_flashdata('success', 'Seller has been created Successfull.');
			redirect('administrator/seller');
		}
	}


	public function updateseller($id = NULL, $page = 'updateseller')

	{
		...
		$data['seller'] = $this->Administrator_Model->get_seller($id);
		...
		if ($this->form_validation->run() === FALSE) {
			...
			$this->Administrator_Model->update_group_data();
	...
	public function deleteseller()
	{
		...
		if (empty($id)) {
			$this->session->set_flashdata('danger', 'Please Select The Item');
			header('Location: ' . $_SERVER['HTTP_REFERER']);
		} else {
			//unset($id[0]); 
			$this->Administrator_Model->deleteseller($id);
			$this->session->set_flashdata('success', 'Seller has been deleted Successfully.');
			header('Location: ' . $_SERVER['HTTP_REFERER']);
		}
	}

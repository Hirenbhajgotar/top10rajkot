...
class CI_Ajax_pagination{
	...
	var $full_tag_open	= '<div class="dataTables_paginate paging_simple_numbers" id="example2_paginate">';
	...
	// var $cur_tag_open	= '&nbsp;<b>';
	// var $cur_tag_close	= '</b>';
	var $cur_tag_open	= '&nbsp;<a class="paginate_button current">';
	var $cur_tag_close	= '</a>';
	var $next_tag_open	= '&nbsp;';
	var $next_tag_close	= '&nbsp;';
	var $prev_tag_open	= '&nbsp;';
	...
	var $num_tag_close	= '';
	var $target         = '';
	var $anchor_class	= 'paginate_button';
	var $show_count     = true;
	var $link_func      = 'getData';
	var $loading        = '.loading';
	var $next_link_class = 'next';
	/**
	 * Constructor
	 * @access	public
	...
	function create_links(){
		...
		if ($num_pages == 1){
            		$info = '<div class="dataTables_info"> Showing : ' . $this->total_rows . '</div>';
			return $info;
		}
		...
		if ($this->show_count){
			...
			$info = '<div class="dataTables_info"> Showing ' . ( $curr_offset + 1 ) . ' to ' ;
			...
			$info .= ' of ' . $this->total_rows . ' entries </div> ';
		...
		if  ($this->cur_page > $this->num_links){
			$output .= $this->first_tag_open 
					. $this->getAJAXlink( '' , $this->first_link, "next")
		...
		if  ($this->cur_page != 1){
			...
			$output .= $this->prev_tag_open 
					. $this->getAJAXlink( $i, $this->prev_link, "previous")
					. $this->prev_tag_close;
		...
		if ($this->cur_page < $num_pages){
			$output .= $this->next_tag_open 
				. $this->getAJAXlink( $this->cur_page * $this->per_page , $this->next_link, "next")
				. $this->next_tag_close;
		}
		// Render the "Last" link
		if (($this->cur_page + $this->num_links) < $num_pages){
			$i = (($num_pages * $this->per_page) - $this->per_page);
			$output .= $this->last_tag_open . $this->getAJAXlink( $i, $this->last_link, "next" ) . $this->last_tag_close;
		}
	...
	function getAJAXlink($count, $text, $class_name = '') {
		
        $pageCount = $count?$count:0;
		
		return '<a href="javascript:void(0);" class="paginate_button ' . $class_name . '" onclick="'.$this->link_func.'('.$pageCount.')">'. $text .'</a>';
	}
}
